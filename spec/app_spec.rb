RSpec.describe App do
  describe '#get_feeds' do
    let(:feeds) { subject.call_rpc(:GetFeeds, {}).feeds }
  ...
  describe '#get_user_feeds' do
    let(:feeds) { subject.call_rpc(:GetUserFeeds, user_id: 123).feeds }
  ...
  describe '#register_feed' do
    let(:created_feed) do
      subject.call_rpc :RegisterFeed, user_id: 123, url: 'https://example.com/feed.json'
    end
  ...
  describe '#refresh_feed' do
    let(:feed_id) { 123 }
    let(:response) { subject.call_rpc(:RefreshFeed, feed_id: feed_id) }
